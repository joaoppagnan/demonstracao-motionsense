...
from ..datasets.motionsense import DeviceMotionMotionSenseDataset

class MotionSenseView:
    def __init__(self, motionsense: DeviceMotionMotionSenseDataset, users: List[int] = None, activities: List[int] = None):
    ...
    def __str__(self):
        ...
        return f"MotionSenseView (no. users={users}, no. activities={activities})"
    
class TimeSeriesMotionSenseView:
    def __init__(self, motionsense: DeviceMotionMotionSenseDataset, users: List[int] = None, activities: List[int] = None, window: int = None, overlap: int = 0):
        self.motionsense = motionsense
        self.users = users
        self.activities = activities
        self.window = window
    ...
    def _create_time_series(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        values = []
        column_names = []
        selected_features = ["attitude.roll", "attitude.pitch", "attitude.yaw",
